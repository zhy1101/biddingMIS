...
def index(request):
    ...
    if (user.uKind == 'PJ' or user.uKind == 'GM'):
        ...
        sps = bidHelp.models.Staff_Project.objects.filter(Q(staff__uID=uID)&Q(project__pState_id__lt=21)).exclude(project__pState_id=8)
        for sp in sps:
            unit = {}
            unit['pro'] = sp.project
...
def showProjectRequest(request, pID):
    ...
    context = {"project": project, "sp_TS": sp_TS, "sp_BS": sp_BS, "sp_SS": sp_SS, 'bidTime': bidTime,
               'bidPlace': bidPlace, 'isPost': isPost, 'docNum': docNum, 'guaMon': guaMon, 'detail': detail,
...
def showBidRequestforST(request, uID):
    ...
    sps = bidHelp.models.Staff_Project.objects.filter(staff__uID = request.session['uID'],project__pState__paramID__lt= 21).exclude(project__pState_id=8)
    projects = []
    for sp in sps:
...
def handleBidPricePredict(request):
    ...
    for project in projects:
        ...
        if (project.startTime.month < 10):
            ...
            month = str(project.startTime.month)
...
def reportResult_win(request, pID):
    ...
    project.pState = state
    project.save()
    bidHelp.models.BidResult.objects.create(pID=project, time=datetime.datetime.now().strftime('%Y-%m-%d'), isWin=True,
                                            winPrice=project.bidPrice)
    bidHelp.models.ProjectProccess.objects.create(pID=project, proccess=state,
                                                  time=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
...
def searchBidResultByWord(request, searchWord):
    searchBidResultSet = []
    if (searchWord != ""):
        searchBidResultSet = bidHelp.models.BidResult.objects.order_by('-time', ).filter(
            Q(pID__pName__contains=searchWord) | Q(pID__inviteID__cID__cName=searchWord) | Q(
                pID__aimDevice__deviceID=searchWord))
    if (len(searchBidResultSet) > 0):
        return render(request, 'bidHelp/BidOpen/recentBidResult.html', {'searchBRSet': searchBidResultSet})
    else:
        return render(request, 'bidHelp/BidOpen/recentBidResult.html',
                      {'searchBRSet': searchBidResultSet, 'remindWord ': 'noResult'})
...
def adminExtralRequest(request):
    ...
    if (uKind == 'PJ'):
        ...
        bidResults = bidHelp.models.BidResult.objects.filter(pID__pState__paramID=9, isWin=True,
                                                             pID__pID__in=myprojectsID)

    for BR in bidResults:
        unit = {}
        unit['bidResult'] = BR
        contract_id = bidHelp.models.Contract.objects.get(pID__pID=BR.pID.pID).contractID
        waitURS = bidHelp.models.ExtraRequest.objects.filter(contractID__contractID=contract_id).exclude(
            Q(answer='Y') | Q(answer='N'))
    ...
    return render(request, 'bidHelp/SignContract/adminExtralRequest.html',
                  {'bidResult_waitURS_yesURS_noURS': bidResult_waitURS_yesURS_noURS})
...
def addURS_handel(request):
    ...
    bidHelp.models.ExtraRequest.objects.create(contractID=contract,
                                               time=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
...
def adminContractDoc(request):
    ...
    for contract in contracts:
        ...
        versionRecords = bidHelp.models.VersionRecord.objects.filter(
            contractID__contractID=contract.contractID).order_by('-contractID', 'verID')
        unit['versionRecords'] = versionRecords
        pro_versionRecords.append(unit)
    return render(request, 'bidHelp/SignContract/adminContractDoc.html',
                  {'contracts': contracts, 'pro_versionRecords': pro_versionRecords})
...
def uploadContract(request):  # 上传文件
    ...
    if request.method == 'POST':
        try:
            ...
            bidHelp.models.VersionRecord.objects.create(contractID=contract, verID=contract.newVerID, docPath=path,
                                                        time=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
...
def downLoadFirstVersionContract(request, conID):
    ...
    if (contract.newVerID == 0):
        contract.newVerID = 1
        contract.save()
        bidHelp.models.VersionRecord.objects.create(contractID=contract, verID=1, docPath='firstVersionContract.zip',
                                                    time=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
...
def toContractCheck(request):
    ...
    for con in contracts:
        unit = {}
        unit['contract'] = con
        processes = bidHelp.models.ProjectProccess.objects.filter(pID__pID=con.pID.pID,
                                                                  proccess__paramID__in=[10, 11, 12])
...
def applyForFirstCheck(request, conID, finalverID, contractPrice, firTimeSpan, firPartPrice, productTime, secTimeSpan,
                       secPartPrice, conveyTime, thrTimeSpan, thrPartPrice):
    contract = bidHelp.models.Contract.objects.get(contractID=conID)
    contract.newVerID = finalverID
    contract.contractDocPath = bidHelp.models.VersionRecord.objects.get(contractID__contractID=conID,
                                                                        verID=finalverID).docPath
    ...
    contract.save()
    contract.pID.save()
    bidHelp.models.ProjectProccess.objects.create(pID=contract.pID, proccess=state,
                                                  time=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
...
def adminReceiveMoney(request):
    ...
    for project in projects:
        ...
        if (project.pState.paramID == 13):
            timeLimit = (contract.signTime + datetime.timedelta(days=contract.firTimeSpan)).strftime(
                '%Y{y}%m{m}%d{d}').format(y='年', m='月', d='日')
            processInfor = '<p>未收款</p><p style = "color:blue">于' + timeLimit + '前，应支付首期货款：¥' + str(
                firMoneyAmount) + '</p>'
            ...
            timeLimit = (overFATTime + datetime.timedelta(days=contract.secTimeSpan)).strftime(
                '%Y{y}%m{m}%d{d}').format(y='年', m='月', d='日')
            processInfor = '<p>已收首期款，共¥' + str(
                firMoneyAmount) + '</p><p style = "color:blue">于' + timeLimit + '前，应支付中期货款：¥' + str(
                secMoneyAmount) + '</p>'
            unit['processInfor'] = processInfor
            ...
            timeLimit = (overSATTime + datetime.timedelta(days=contract.thrTimeSpan)).strftime(
                '%Y{y}%m{m}%d{d}').format(y='年', m='月', d='日')
            processInfor = '<p>已收中期款，共¥' + str(
                oneAndTwoMoneyAmount) + '</p><p style = "color:blue">于' + timeLimit + '前，应支付尾款：¥' + str(
                thrMoneyAmount) + '</p>'
            unit['processInfor'] = processInfor

        pro_contract_processInfor.append(unit)

    return render(request, 'bidHelp/ExerciseAgreement/adminReceiveMoney.html',
                  {'pro_contract_processInfor': pro_contract_processInfor})
...
def adminProductConvey(request):
    ...
    for project in projects:
        ...
        pro_contract_processInfor.append(unit)
    return render(request, 'bidHelp/ExerciseAgreement/adminProductConvey.html',
                  {'pro_contract_processInfor': pro_contract_processInfor})
...
def adminWarning(request,dayarea):
    if(not dayarea):
        dayarea = 3
    else:
        dayarea = int(dayarea)
    ...
    for project in projects:
        ...
        if (project.pState_id < 5 ):
            ...
            delta = (pre_bidOpenTime - datetime.datetime.now()).days + 1
            ...
            if ('date' in unit.keys()):
                pro_alarmKind_predate_date.append(unit)

        elif (project.pState_id >= 10 and project.pState_id < 14 and bidHelp.fastGetter.getBidResultByPID(
                project.pID) == 1):
            contract = bidHelp.fastGetter.getContractByPID(project.pID)
            ...
            if (signTime != ""):
                ...
                delta = (pre_firstPriceDate - datetime.date.today()).days +1
    ...
    return render(request, 'bidHelp/ExerciseAgreement/alarmPage.html',
                  {'pro_alarmKind_predate_date': pro_alarmKind_predate_date})
...
def overProject(request, pID):
    ...
    project.save()
    contract.save()
    bidHelp.models.ProjectProccess.objects.create(pID=project, proccess=bidHelp.models.StateParam.objects.get(paramID=21),
                                                  time=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
...
def checkCustomer(request):
    ...
    if (len(projects) > 0):
        ...
        winBidResults = bidHelp.models.BidResult.objects.filter(pID__inviteID__cID=cID, isWin=True,
                                                                pID__pState__paramID__gt=10)
